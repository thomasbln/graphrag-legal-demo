...
export function GraphRAGColumn({ queryId, results, isLoading }: GraphRAGColumnProps) {
  if (isLoading) {
    return (
      <div className="border-2 border-cyan-400/50 dark:border-cyan-500/50 rounded-2xl p-6 bg-white dark:bg-slate-900/95 backdrop-blur-xl shadow-xl">
        ...
        <div className="flex items-center justify-center py-12 mb-4">
          <div className="relative w-24 h-24">
            ...
            <div className="absolute bottom-0 left-0 w-4 h-4 rounded-full bg-cyan-400 animate-pulse" style={{ animationDelay: '0.2s' }}></div>
            <div className="absolute bottom-0 right-0 w-4 h-4 rounded-full bg-cyan-400 animate-pulse" style={{ animationDelay: '0.4s' }}></div>
            
            {/* Connections (animated lines) */}
            <svg className="absolute inset-0 w-full h-full" style={{ overflow: 'visible' }}>
              <line x1="50%" y1="0" x2="0" y2="100%" stroke="rgb(6 182 212)" strokeWidth="2" strokeDasharray="4" className="animate-pulse" opacity="0.5" />
              <line x1="50%" y1="0" x2="100%" y2="100%" stroke="rgb(6 182 212)" strokeWidth="2" strokeDasharray="4" className="animate-pulse" opacity="0.5" style={{ animationDelay: '0.2s' }} />
            </svg>
          </div>
        </div>
          ...
          <>
            ...
            {queryId && (
              <ResultExplanation 
                ...
                hasAggregations={!!(results.result.aggregations && Object.keys(results.result.aggregations).length > 0)}
              />
            )}
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-sm font-bold text-slate-900 dark:text-slate-100">
                    Gefundene Klauseln ({results.result.clauses.length})
                  </h3>
                  <Badge variant="success" className="text-xs">
                    GraphRAG zeigt Beziehungen
                  </Badge>
                </div>
                <div className="space-y-2">
                  {results.result.clauses.slice(0, 8).map((item: any, idx: number) => (
                    <div key={idx} className="p-3 bg-cyan-50 dark:bg-cyan-950/20 rounded-lg border border-cyan-200 dark:border-cyan-800/50">
                      <div className="flex items-start justify-between gap-2 mb-2">
                        <Badge variant="success" className="text-xs">
                          {item.clauseType.name}
                        </Badge>
                        <span className="text-xs text-slate-500 dark:text-slate-400 truncate flex-1 text-right">
                          aus: {item.contract.title}
                        </span>
                      </div>
                  ...
                  {results.result.clauses.length > 8 && (
                    <p className="text-xs text-slate-500 dark:text-slate-400 text-center pt-2">
                      ... und {results.result.clauses.length - 8} weitere Klauseln in den Vertragsdetails
                    </p>
                  )}
                </div>
              </div>
            ...
            {results.result.contracts && results.result.contracts.length > 0 && (
              <div className="space-y-3">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-sm font-bold text-slate-900 dark:text-slate-100">
                    Gefundene Verträge ({results.result.contracts.length})
                  </h3>
                  <span className="text-xs text-slate-500 dark:text-slate-400 font-medium">
                    {results.result.contracts.length} gefunden
                  </span>
                </div>
                <div className="space-y-2">
                  {results.result.contracts.slice(0, 5).map((contract: any) => {
                    // Finde Clauses für diesen Contract
                    const contractClauses = results.result.clauses?.filter(
                      (c: any) => c.contract.id === contract.id || 
                                  c.contract.title === contract.title ||
                                  (!c.contract.id && c.contract.title === contract.title)
                    ) || []
